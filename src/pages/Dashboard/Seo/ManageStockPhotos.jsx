...
const ManageStockPhotos = () => {
  ...
  queryClient.invalidateQueries({ queryKey: ["manage-stock-photos"] });
  const deleteAsset = (key) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.delete(`/${stockPhotosRootURL}?key=${key}`).then((res) => {
  ...
  const {
    ...
    queryFn: async () => {
      let url = "";
      if (searchTerm) {
        url = `/${stockPhotosRootURL}/${ByAdmin}?searchTerm=${searchTerm}&page=${currentPage}&limit=${pageLimit}`;
      } else {
        url = `/${stockPhotosRootURL}/${ByAdmin}?page=${currentPage}&limit=${pageLimit}`;
      }
    ...
    <div>
      <div className=" flex justify-end items-center gap-2">
        ...
        <Helmet>
          ...
          <meta name="description" content="Stock Photos" />
        </Helmet>
...
export default ManageStockPhotos;
