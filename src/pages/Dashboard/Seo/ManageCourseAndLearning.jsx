...
const ManageCourseAndLearning = () => {
  ...
  queryClient.invalidateQueries({ queryKey: ["manage-courses-learning"] });
  const deleteAsset = (key, previewKey) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure
          .delete(
            `/${courseAndLearningRootURL}/delete-file?key=${key}&previewKey=${previewKey}`
          )
          .then((res) => {
  ...
  const {
    ...
    queryFn: async () => {
      let url = "";
      if (searchTerm) {
        url = `/${courseAndLearningRootURL}/${ByAdmin}?searchTerm=${searchTerm}&page=${currentPage}&limit=${pageLimit}`;
      } else {
        url = `/${courseAndLearningRootURL}/${ByAdmin}?page=${currentPage}&limit=${pageLimit}`;
      }
    ...
    <div>
      <div className=" flex justify-end items-center gap-2">
        ...
        <Helmet>
          ...
          <meta name="description" content="Course And Learning" />
        </Helmet>
...
export default ManageCourseAndLearning;
