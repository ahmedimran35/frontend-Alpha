...
const DesignTemplateSingleAsset = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosPublic.get(
        `${designTemplateRootURL}/details-check-by-user/${assetId}`
      );
  ...
  const designTemplateFileDownloadHandler = async (key, originalFileName) => {
    try {
      setLoading(true);

      const bucketName = "design-template";
      const downloadInfo = {
        assets: assetId,
            ...
            {userLoading ? (
              ...
              <div className="flex flex-row gap-2 mt-5">
                {loading ? (
                  ...
                  <button
                    onClick={() =>
                      designTemplateFileDownloadHandler(
                        assetDetail.key,
                        assetDetail?.title
                      )
                    }
...
export default DesignTemplateSingleAsset;
