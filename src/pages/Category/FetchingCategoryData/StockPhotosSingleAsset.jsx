...
import { stockPhotosRootURL } from "../../../utils/Constants/decryptedApiConstants/apiURL";
...
const StockPhotosSingleAsset = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosPublic.get(
        `${stockPhotosRootURL}/details-check-by-user/${assetId}`
      );
  ...
  // const category = assetDetail?.category;

  const stockPhotoFileDownloadHandler = async (key, originalFileName) => {
    try {
      setLoading(true);

      const bucketName = "stock-photos";
      const downloadInfo = {
        assets: assetId,
...
export default StockPhotosSingleAsset;
